...
@implementation AppDetailsControllerPad
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (self.application.link != nil && [self.application.link length] != 0) {    
        shareButtonInside = [UIButton buttonWithType:UIButtonTypeCustom];
        [shareButtonInside setFrame:CGRectMake(0, 0, 26, 19)];
        [shareButtonInside addTarget:self action:@selector(actions:) forControlEvents:UIControlEventTouchUpInside];
        shareButtonInside.backgroundColor = [UIColor clearColor];
        shareButtonInside.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin; 
        [shareButtonInside setImage:[UIImage imageNamed:@"actionIcon.png"] forState:UIControlStateNormal];
        shareButton = [[UIBarButtonItem alloc] initWithCustomView:shareButtonInside];
...
- (IBAction) editStores {
    ...
    if (![app.rightNav.visibleViewController isKindOfClass:[ReviewsControllerPad class]]) {
        ...
        if ([app.rightNav.visibleViewController isKindOfClass:[StoreSelectionControllerPad class]]) {
            animated = NO;            
        }
    ...
    StoreSelectionControllerPad* ssc = [[[StoreSelectionControllerPad alloc] initWithNibName:@"StoreSelectController" 
                                                                                      bundle:nil 
                                                                                      father:self 
...
- (IBAction) actions:(id)sender {
    ...
    [popupQuery showFromBarButtonItem:shareButton animated:YES];
    
    [shareButtonInside removeTarget:nil 
                       action:NULL 
